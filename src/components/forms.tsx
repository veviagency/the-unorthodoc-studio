...
export function DemoAction({ label, note = "This concept is ready to connect to checkout." }: { label: string; note?: string }) {
  const [done, setDone] = useState(false);
  if (done) return <div className="demo-success" role="status"><Check/><span><strong>Perfect.</strong> {note}</span></div>;
  return <Button size="lg" variant="editorial" onClick={() => setDone(true)}>{label}<ArrowRight/></Button>;
}
...
export function ContactForm() {
  ...
  return <form className="contact-form" onSubmit={submit}>
    ...
    <div className="form-row"><label><span>Organization</span><input required name="organization" /></label><label><span>Inquiry type</span><select required defaultValue=""><option value="" disabled>Select one</option><option>Strategic partnership</option><option>Speaking or media</option><option>Media kit request</option><option>Other</option></select></label></div>
